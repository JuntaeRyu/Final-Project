...
public class MemberProfileDAO2 implements InterfaceMemberProfileDAO{
	...
	// 회원 프로필 전체 출력
	private final String sql_SELECTALL_HELCELL = "SELECT MP.PROFILENUM, MP.MEMBERID, MP.PROFILEIMG, MP.SHORTINTRO, MP.INTRO, MP.PROHIBITCNT, M.NICKNAME, (SELECT COUNT(CASE WHEN COMMONNUM = MP.PROFILENUM THEN 1 END) FROM RECOMMEND) AS RECOMMENDCNT FROM MEMBERPROFILE MP JOIN MEMBER M ON MP.MEMBERID=M.MEMBERID WHERE M.ROLE = 3 ORDER BY DBMS_RANDOM.VALUE";
	// 메인페이지 회원프로필 4개 출력
